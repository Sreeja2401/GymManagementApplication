...
pipeline {
    ...
    environment {
        SONAR_TOKEN = credentials('GymManagement')
    ...
    stages {
        ...
        stage('SonarQube Analysis') {
            steps {
                bat "mvn sonar:sonar -Dsonar.projectKey=GymManagement -Dsonar.projectName='GymManagement' -Dsonar.host.url=http://localhost:9000 -Dsonar.login=%SONAR_TOKEN% -Dsonar.java.coveragePlugin=jacoco"
            }
            post{
            success{
